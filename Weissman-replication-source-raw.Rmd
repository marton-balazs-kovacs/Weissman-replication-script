...
* 1: The recorded response is only a trial response
* 2: The unique identifier is missing
* 3: The responses is collected during the first wave of the data collection in Hungary
...
# Participant 393 somehow completed the stroop task and the primeprobe task as well
# Originally he recieved invitation for the primeprobe
```
...
```{r save tasks}
write_tsv(unnest(source$join[[1]]), path = paste0("data/Raw/", source$task[[1]], "/", source$task[[1]], "_data.tsv"))

write_tsv(unnest(source$join[[2]]), path = paste0("data/Raw/", source$task[[2]], "/", source$task[[2]], "_data.tsv"))

write_tsv(unnest(source$join[[3]]), path = paste0("data/Raw/", source$task[[3]], "/", source$task[[3]], "_data.tsv"))

write_tsv(unnest(source$join[[4]]), path = paste0("data/Raw/", source$task[[4]], "/", source$task[[4]], "_data.tsv"))
```