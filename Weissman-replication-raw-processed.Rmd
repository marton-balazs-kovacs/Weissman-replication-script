...
## TODO: Write code to filter out participants first who only have TIMEOUT trials because mean function returns NaN in that case like participant_id == 80
         ...
         response = map(response,
                        . %>% 
                          mutate(dropTrialRt = case_when(rtParticipantMean + 2.5 * rtParticipantSd < rtConditionalMean ~ 1L,
                                                         ...
                                                         is.na(rtParticipantMean) & is.na(rtParticipantMean) ~ 1L,
                                                         TRUE ~ 0L))))
raw$rtTrialOutliers[[1]] %>% View()
raw$response[[1]] %>% View()
```
