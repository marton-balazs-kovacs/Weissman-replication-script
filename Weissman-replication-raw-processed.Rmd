...
raw <-
  ...
  mutate(rtParticipantOutliers = map(response,
                                     . %>%
                                      ...
                                      #add %>% filter(!is.na(isPrevCongruent)) to the end here if previous chunk was commented out
                                      #to make sure rtParticipantOutliers and rtTrialOutliers are the same size
         rtTrialOutliers = map(response,
                               . %>% 
                                 ungroup() %>% 
                                 #filter(!is.na(isPrevCongruent)) %>% # this is only necessary if the previous chunk was commented out
  ...
  mutate(response = map(response,
                        . %>% 
                          ...
                          mutate(dropTrialRt = case_when(rtConditionalMean + 2.5 * rtConditionalSd < rt ~ 1L,
                                                         ...
                                                         TRUE ~ 0L),
                                 isPrevDropTrialRt = case_when(trial <= 2 ~ NA_integer_, #at this point there should be no trial == 1 trials because they were removed when is.na(PrevCongruent) was filtered, trial == 2 trials will have NO previous trial as the immediately preceding trial will be the last trial of the previous block; NOTE: if we want to keep the second trial of each block, we can change this line to 
                                                               #trial == 2 ~ 1L, 
