...
raw <-
  ...
  mutate(rtParticipantOutliers = map(response,
         ...
         response = pmap(response, rtParticipantOutliers, rtTrialOutliers, 
                         ~ left_join(.x, .y, by = "participant_id") %>% 
                           left_join(., .z, by = c("participant_id", "isCongruent", "isPrevCongruent"))))
```
...
raw <-
  ...
  mutate(responseRtAnalysis = map(response,
                                  ~ filter(.,
                                           !is.na(PrevCongruent),     # 1 exclusion criteria
...
# Exclude participants younger than 18
```{r}
raw <-
  raw %>% 
  map(response,
