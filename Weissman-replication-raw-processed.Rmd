...
* 1: First trial for each participant - IN EVERY BLOCK (Máté)
...
#this should now remove the first trial of every block too
```
  ...
  mutate(response = map(response,
                        . %>% 
                          ...
                          mutate(dropTrialRt = case_when(rtConditionalMean + 2.5 * rtConditionalSd < rt ~ 1L,
                                                         ...
                                                         TRUE ~ 0L),
                                 isPrevDropTrialRt = case_when(trial <= 2 ~ NA_integer_, #at this point there should be no trial == 1 trials because they were removed is.na(PrevCongruent) was filtered, trial == 2 trials will have NO previous trial as the immediately preceding trial will be the last trial of the previous block
