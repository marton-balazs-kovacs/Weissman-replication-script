...
# Reading in original study effect sizes
...
processed <-
  ...
  mutate(anova_rt_bf_effect = map2(anova_rt_raw_effect, original_effect,
...
If the Bayes factor is greater than the preset threshold of B = 3 for the interaction effect in a given task we run a simple effects analyses, contrasting post-congruent congruent (cC) and post-incongruent congruent (iC) trials in one analysis, and post-congruent incongruent (cI) and post-incongruent incongruent (iI) trials in another.
...
processed <-
  ...
  mutate(corr_bf_effect = map(corr,
                              ~ tibble(estimateFisherZ = DescTools::FisherZ(.x$estimate),
                                       nParticipant = .x$parameter,
                                       se = 1 / sqrt(nParticipant - 1))))
```
...
processed <-
  ...
  mutate(anova_acc_bf_effect = map2(anova_acc_raw_effect, original_effect,
                              ~ left_join(.x, .y, by = "term")))
```
