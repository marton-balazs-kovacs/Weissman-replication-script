...
pacman::p_load(tidyverse,osfr,lme4,BayesFactor,DescTools,sjstats,car,broom)
```
...
processed$acc_data <- set_names(processed$rt_data, processed$task)
```
...
processed <-
  ...
  mutate(cse_plot_acc_data = map(acc_data,
         ...
         cse_plot_acc = map(cse_plot_acc_data,
...
Bf(sd = post_congruent$estimate / post_congruent$statistic,
   obtained = post_congruent$estimate,
   dfdata = post_congruent$parameter,
   meanoftheory = 0,
...
Bf(sd = post_incongruent$estimate / post_incongruent$statistic,
   obtained = post_incongruent$estimate,
   dfdata = post_incongruent$parameter,
   meanoftheory = 0,
   sdtheory = processed$anova_rt_bf_effect[[1]]$originalRawEffect[1] / 4,
   dftheory = 10^10,
   tail = 1)
...
##### Preprocessing data for the Bf analysis
...
```{r}
# TODO: Calculate Bf for Dunn test mean rank differences
```
