...
participant_duplicate_drop <-
  ...
  filter(id %in% c(82, 84)) %>% #These numbers come from line 153 of Weissman-replication-raw-processed 
...
#Preprocess data
#THE AIM IS TO HAVE 118 PARTICIPANTS (this bit's okay), WITH 33239 TRIALS BETWEEN THEM (this bit is not)

# First create previous trial congruency and accuracy
#NOTE - IMPORTANT: I am doing this the way I think it's done
#in Weissman-repplication-raw-processed - however, I am not
...
for (d in 1:length(merged$subid)) {
  if (merged$trialid[d] < 2) { #I think this should actually be merged$trial[d]
    merged$precong[d] <- NA
    merged$preacc[d] <- NA
  } else {
...
#SAME NOTE AS ABOVE
merged <- merged %>% mutate(trim = ifelse(trialid>1,RT,NA_real_)) #this should be (trial > 1)
...
for (b in 1:length(merged$subid)) {
  if (merged$trialid[b] < 2) { #this should be merged$trial[b]
    merged$post_outlier[b] <- NA
  } else {
    if (merged$trialid[b] == 2) {merged$post_outlier[b] <- 0} #this should be merged$trial[b]
